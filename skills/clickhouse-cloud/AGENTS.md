...
Step-by-step guides for working with ClickHouse Cloud using the chv CLI. 17 guides across 7 sections covering setup, local development, service management, sizing, migration, security, and backups.
...
2. [Local Development](#2-local-development)
   ...
   - [Run SQL Queries with chv](#run-sql-queries-with-chv)
...
4. [Sizing & Scaling](#4-sizing--scaling)
...
5. [Local to Cloud](#5-local-to-cloud)
...
**Prerequisites:** chv CLI installed, a ClickHouse Cloud account with an API key.
...
Create a project-local ClickHouse data directory for isolated development.

**Prerequisites:** chv CLI installed with at least one ClickHouse version.

**Steps:**

1. **Initialize the local data directory**
...
Execute SQL queries using `chv run` without starting a persistent server.
...
Move your schema and data from a local ClickHouse development environment to a ClickHouse Cloud service.

**Prerequisites:** A local ClickHouse project with data and a Cloud service created.
...
4. **Get cloud service connection details**

```bash
chv cloud --json service get <service-id>
```

5. **Apply schema to cloud**
...
- For large datasets, use Native format for better performance
- Export and import one table at a time
